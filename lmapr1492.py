...
def go_points(points, size=4, color="black", labels=None):
    ...
    if labels is not None:
        ...
        for il in range(len(labels)):
            if labels[il] in replace:
                labels[il] = replace[labels[il]]
...
def get_branch_labels(bs, i_branch):
    ...
    if start_label[0] == "\\" or "_" in start_label:
        start_label = "$" + start_label + "$"
    if end_label[0] == "\\" or "_" in end_label:
        end_label = "$" + end_label + "$"
